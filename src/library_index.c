...
#define BUILD_LOG_MAX 64
#define BUILD_LOG_LINE 128
static char build_log[BUILD_LOG_MAX][BUILD_LOG_LINE];
static int build_log_count = 0;

static volatile bool search_abort = false;
static volatile bool search_timed_out = false;

static void append_build_log_locked(const char* msg) {
    if (!msg || !msg[0]) return;
    if (build_log_count < BUILD_LOG_MAX) {
        snprintf(build_log[build_log_count], BUILD_LOG_LINE, "%s", msg);
        build_log_count++;
        return;
    }
    memmove(build_log[0], build_log[1], (BUILD_LOG_MAX - 1) * BUILD_LOG_LINE);
    snprintf(build_log[BUILD_LOG_MAX - 1], BUILD_LOG_LINE, "%s", msg);
}

static void set_status(const char* msg) {
    pthread_mutex_lock(&index_mutex);
    snprintf(build_status, sizeof(build_status), "%s", msg ? msg : "");
    append_build_log_locked(msg);
...
bool LibraryIndex_isFailed(void) {
    pthread_mutex_lock(&index_mutex);
    bool failed = build_started && !build_running && !index_ready;
    pthread_mutex_unlock(&index_mutex);
    return failed;
}

bool LibraryIndex_canSearch(void) {
    return LibraryIndex_isReady();
}
...
IndexBuildState LibraryIndex_getBuildState(void) {
    if (LibraryIndex_isBuilding()) return INDEX_STATE_BUILDING;
    if (LibraryIndex_isFailed()) return INDEX_STATE_FAILED;
    if (LibraryIndex_isReady()) return INDEX_STATE_READY;
...
const char* LibraryIndex_getBuildLogLine(int index) {
    ...
    if (index >= 0 && index < build_log_count) {
        line = build_log[index];
    }
    pthread_mutex_unlock(&index_mutex);
    return line;
}
